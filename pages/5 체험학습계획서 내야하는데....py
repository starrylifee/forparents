...
# 필수 입력 필드
user_grade = st.selectbox("학년을 선택하세요. <span style='color: red;'>(필수)</span>", ["1학년", "2학년", "3학년", "4학년", "5학년", "6학년"], unsafe_allow_html=True)
start_date = st.date_input("학습 시작일을 선택하세요. <span style='color: red;'>(필수)</span>", unsafe_allow_html=True)
end_date = st.date_input("학습 종료일을 선택하세요. <span style='color: red;'>(필수)</span>", unsafe_allow_html=True)
...
purpose = st.text_area("체험학습의 목적을 입력하세요. <span style='color: red;'>(필수)</span>", unsafe_allow_html=True)
location = st.text_input("체험학습 장소를 입력하세요. 예시: 국립과학관 <span style='color: red;'>(필수)</span>", unsafe_allow_html=True)
user_email = st.text_input("체험학습 계획서를 받을 이메일을 입력하세요. <span style='color: red;'>(필수)</span>", unsafe_allow_html=True)
    ...
    if st.button("수정한 계획서를 이메일로 보내기"):
        with st.spinner("이메일을 보내는 중입니다..."):
            if 'ai_response' not in st.session_state or not st.session_state['ai_response']:
                ...
                print("[DEBUG] No AI response available to send")
            elif not user_email:
